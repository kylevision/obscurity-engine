"""OBSCURITY ENGINE v7.0 — FINAL FORM"""
import os,sys,json,random,logging,re
...
from modules.brute_force import *"""OBSCURITY ENGINE v7.0 — FINAL FORM"""
import os,sys,json,random,logging,re
from pathlib import Path
from datetime import datetime,date,timedelta
from collections import Counter
...
def _yt(svc,qs,mpq,pa,pb,so,da,rg,la,ca,ss,vd,et=""):
    ...
    for i,q in enumerate(qs):
        ...
        while fetched_count < mpq:
            # We can request max 50 per page from API
            ...
            r=search_youtube(
                ...
                safe_search=ss,
                event_type=et,
                page_token=next_page_token # Pass the token to get next page
            )
            
            if r.get("error"):
                st.warning(f"API Error: {r['error']}")
                break
    ...
    # Process details in batches of 50 (API limit for details)
    if u:
        all_details = {}
        ...
        for idx, chunk in enumerate(u_chunks):
            ...
            det=get_video_details(svc,vs)
            if det: all_details.update({d['id']: d for d in det})
            
        # We need to reconstruct the details list from the dict to pass to parse_video_data
...
def render_search():
    ...
    with st.expander("CONTROL PANEL",expanded=False):
        ...
        with tf:
            ...
            # EDITED: Switched to number_input and increased max to 500
    ...
    if should:
        ...
        if ssc:
            ...
            for i,q in enumerate(aq):
                ...
                # EDITED: Removed min(mpq, 20) limit
...
def render_setup():
    ...
    with t1:
        ak=get_all_api_keys()
        if ak:st.success(f"{len(ak)} KEYS");[st.code(f"K{i+1}:{k[:8]}...{k[-4:]}")for i,k in enumerate(ak)]
...
if __name__=="__main__":main()"""OBSCURITY ENGINE v7.0 — FINAL FORM"""
import os,sys,json,random,logging,re
from pathlib import Path
from datetime import datetime,date,timedelta
from collections import Counter
...
def _yt(svc,qs,mpq,pa,pb,so,da,rg,la,ca,ss,vd,et=""):
    ...
    for i,q in enumerate(qs):
        p.progress((i+1)/len(qs),f"SYS> '{q}' [{i+1}/{len(qs)}]")
        r=search_youtube(service=svc,query=q,max_results=mpq,published_after=pa,published_before=pb,order=so,video_duration=da,region_code=REGION_CODES.get(rg,""),relevance_language=RELEVANCE_LANGUAGES.get(la,""),video_category_id=VIDEO_CATEGORIES.get(ca,""),video_definition=vd,safe_search=ss,event_type=et)
        if r.get("error"):st.warning(r["error"])
        ai.extend(r["items"]);st.session_state.search_count+=1
    seen=set();u=[]
    for it in ai:
        vid=it.get("id",{});vi=vid.get("videoId","") if isinstance(vid,dict) else str(vid)
        if vi and vi not in seen:seen.add(vi);u.append(it)
...
def render_search():
    ...
    with st.expander("CONTROL PANEL",expanded=False):
        ...
        with tf:
            ...
            with f1:mnv=st.number_input("MIN_V",0,10000000,0);oz=st.checkbox("ZERO")
            with f2:das=st.selectbox("DUR",["any","short","medium","long"]);mnd=st.number_input("MIN_S",0,999999,0,step=5);mxd=st.number_input("MAX_S",0,999999,999999,step=60)
            with f3:so=st.selectbox("SORT",["date","viewCount","relevance","rating"]);mpq=st.slider("PER_Q",5,50,25);od=st.checkbox("DEF")
            with f4:mw=st.slider("MIN_W",0,80,0,step=5);gh=st.checkbox("GHOST");geo=st.checkbox("GEO");vdf=st.selectbox("DEF",["any","high","standard"],key="vdf")
        with tr:
            r1,r2,r3=st.columns(3)
            with r1:rg=st.selectbox("REG",list(REGION_CODES.keys()))
    ...
    if should:
        ...
        if ssc:
            ...
            for i,q in enumerate(aq):
                sp2.progress((i+1)/len(aq),f"SCRAPE '{q}'")
                sdf=scraper_search_full(q,min(mpq,20))
                if not sdf.empty:sdf=analyze_thumbnails(sdf);sdfs.append(sdf)
            if sdfs:
                sdf=pd.concat(sdfs,ignore_index=True).drop_duplicates(subset=["video_id"]).reset_index(drop=True);emv=0 if oz else tmv;sdf=_pf(sdf,emv,**fkw)
                if not sdf.empty:sdf=sdf.sort_values("weirdness_score",ascending=False).reset_index(drop=True)
